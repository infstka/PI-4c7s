<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="WebForm.aspx.cs" Inherits="LR4.WebForm" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
    <title>LR4</title>
    <script src="jquery.js"></script>
    <script>
        function Sum() {
            const data = {
                x: parseInt($("#x").val()),
                y: parseInt($("#y").val())
            };
            // асинхронный запрос - позволяет получать данные без перезагрузки страницы в фоне
            $.ajax({
                url: "Simplex.asmx/Sum_4",
                type: "GET",
                dataType: "json", // ожидаемый тип ответа
                contentType: "application/json; charset=utf-8",
                data: data,
                success: result => {
                    $("#result").val(result.d);
                },
                error: error => {
                    console.log(error);
                }
            })
        }
    </script>
</head>
<body>
    <form id="sumform" runat="server">
        <div style="margin-bottom: 10px;">
            <div style="margin-bottom: 10px;">
                <input type="text" id="x" placeholder="x" />
                <input type="text" id="y" placeholder="y" />
                <input type="button" onclick="Sum()" value="Calculate" />
            </div>
            <div>
                <input type="text" id="result" />
            </div>
        </div>
    </form>
</body>
</html>
